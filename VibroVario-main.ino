...
void readRTC() {
  ...
  Wire.requestFrom((int)ADDR_RTC, (int)7);
  if(Wire.available()) {
...
void setRTCAlarmSec(int secFromNow) {
  Wire.beginTransmission(ADDR_RTC); Wire.write(0x02); Wire.endTransmission();
  Wire.requestFrom((int)ADDR_RTC, (int)6);
  ...
  Wire.requestFrom((int)ADDR_RTC, (int)1);
  uint8_t cs1 = 0;
...
void initSensors() {
    ...
    if (Wire.endTransmission() == 0) {
        Wire.requestFrom((int)ADDR_BMA, (int)1);
    ...
    for(;;) {
        if (fsmStateRTC == FSM_RUNNING && data.sensInit && vfsm.running) {
            ...
            if (data.accInit && accSafe) {
                ...
                Wire.requestFrom((int)ADDR_BMA, (int)6);
                if (Wire.available() >= 6) {
