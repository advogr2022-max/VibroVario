...
#define CLOCK_SLEEP_SEC   60            // Сон между обновлениями часов (сек)
#define FLIGHT_DETECT_VZ  0.5f          // Порог Vz для автостарта полёта (м/с)
#define FLIGHT_DETECT_SEC 3             // Сколько секунд Vz > порога для старта
#define LANDING_DETECT_SEC 30           // Сколько секунд без движения = посадка
#define DEEP_SLEEP_TIMEOUT 300          // 5 мин без движения → полный сон (сек)
...
RTC_DATA_ATTR int lastWakeMinute = -1;                // Минута последнего пробуждения часов (чтобы не обновлять экран чаще 1мин)
...
void loop() {
    // === CLOCK MODE: показ часов, сон 1 мин ===
    if (state == CLOCK) {
        // Устанавливаем RTC alarm на +60 секунд
        setRTCAlarmSec(CLOCK_SLEEP_SEC);
        // Настраиваем BMA any-motion wake (на случай если ещё не настроен)
        initBMAMotionWake();
        // Глубокий сон с пробуждением от RTC/BMA/кнопки
        goDeepSleep();
        // Сюда не доходим — выше ESP перезагружается
    }
